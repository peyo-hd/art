...
static constexpr ManagedRegister kXArgumentRegisters[] = {
    Riscv64ManagedRegister::FromGpuRegister(A0),
    Riscv64ManagedRegister::FromGpuRegister(A1),
    Riscv64ManagedRegister::FromGpuRegister(A2),
    Riscv64ManagedRegister::FromGpuRegister(A3),
    Riscv64ManagedRegister::FromGpuRegister(A4),
    Riscv64ManagedRegister::FromGpuRegister(A5),
    Riscv64ManagedRegister::FromGpuRegister(A6),
    Riscv64ManagedRegister::FromGpuRegister(A7),
};
...
ArrayRef<const ManagedRegister> Riscv64JniCallingConvention::CalleeSaveScratchRegisters() const {
  ...
  // Use S2-S9 from native callee saves.
  constexpr size_t kStart = 12u;
  constexpr size_t kLength = 8u;
  static_assert(kCalleeSaveRegisters[kStart].Equals(
                    Riscv64ManagedRegister::FromGpuRegister(S2)));
...
ArrayRef<const ManagedRegister> Riscv64JniCallingConvention::ArgumentScratchRegisters() const {
  ...
  static_assert(kXArgumentRegisters[0].Equals(Riscv64ManagedRegister::FromGpuRegister(A0)));
