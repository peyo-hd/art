...
static constexpr size_t kRiscv64StackAlignment = 16u;
static_assert(kRiscv64StackAlignment == kStackAlignment);
...
inline size_t GetNativeOutArgsSize(size_t num_fp_args, size_t num_non_fp_args) {
  ...
  if (num_non_fp_args < kMaxIntLikeRegisterArguments)
    num_stack_fp_args -=
        std::min((kMaxIntLikeRegisterArguments - num_non_fp_args), num_stack_fp_args);
...
inline size_t GetCriticalNativeStubFrameSize(const char* shorty, uint32_t shorty_len) {
  ...
  return RoundUp(size, kRiscv64StackAlignment);
}
